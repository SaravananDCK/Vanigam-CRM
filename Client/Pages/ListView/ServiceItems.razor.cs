...
namespace Vanigam.CRM.Client.Pages.ListView
{
    public partial class ServiceItems
    {
        ...
        {
            return new ODataFilter<ServiceItem>()
        ...
        {
            await DialogService.OpenDialogAsync<EditServiceItem>(Localizer["AddServiceItem"], null, 100, 100);
            await GridReload();
        }

        protected async Task EditRow(DataGridRowMouseEventArgs<ServiceItem> args)
        {
        ...
        {
            await DialogService.OpenDialogAsync<EditServiceItem>(Localizer["EditServiceItem"], new Dictionary<string, object> { { "Oid", serviceItem.Oid } }, 100, 100);
            await GridReload();
        }

        protected async Task GridDeleteButtonClick(ServiceItem serviceItem)
        {
            try
            {
                if (await DialogService.Confirm(Localizer["DeleteRecord"]) == true)
                {
                    var deleteResult = await ServiceItemApiService.Delete(oid:serviceItem.Oid);
