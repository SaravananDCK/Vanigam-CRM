...
{
    public partial class EditQuoteItem
    {
        [Inject] private QuoteItemApiService QuoteItemApiService { get; set; }
        protected override async Task OnInitializedAsync()
        {
            if (Oid == Guid.Empty)
                CurrentObject = new();
            else
                CurrentObject = await QuoteItemApiService.GetByOid(oid: Oid);
        ...
        protected async Task FormSubmit()
        {
            ...
            try
            {
                if (Oid == Guid.Empty)
                {
                    CurrentObject = await QuoteItemApiService.Create(CurrentObject);
                }
                else
                {
                    var result = await QuoteItemApiService.Update(oid: Oid, CurrentObject);
