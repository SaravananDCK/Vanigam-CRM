...
namespace Vanigam.CRM.Client.Pages.DetailView
{
    ...
    {
        [Inject] private NotificationApiService NotificationApiService { get; set; }
        protected override async Task OnInitializedAsync()
        {
            if (Oid == Guid.Empty)
                CurrentObject = new();
            else
                CurrentObject = await NotificationApiService.GetByOid(oid: Oid);
        ...
        protected async Task FormSubmit()
        {
            ...
            try
            {
                if (Oid == Guid.Empty)
                {
                    CurrentObject = await NotificationApiService.Create(CurrentObject);
                }
                else
                {
                    var result = await NotificationApiService.Update(oid: Oid, CurrentObject);
