...
namespace Vanigam.CRM.Client.Pages.DetailView
{
    ...
    {
        [Inject] private AttachmentApiService AttachmentApiService { get; set; }
        protected override async Task OnInitializedAsync()
        {
            if (Oid == Guid.Empty)
                CurrentObject = new();
            else
                CurrentObject = await AttachmentApiService.GetByOid(oid: Oid);
        ...
        protected async Task FormSubmit()
        {
            ...
            try
            {
                if (Oid == Guid.Empty)
                {
                    CurrentObject = await AttachmentApiService.Create(CurrentObject);
                }
                else
                {
                    var result = await AttachmentApiService.Update(oid: Oid, CurrentObject);
