...
{
    public partial class EditGPSPoint
    {
        [Inject] private GPSPointApiService GPSPointApiService { get; set; }
        protected override async Task OnInitializedAsync()
        {
            if (Oid == Guid.Empty)
                CurrentObject = new();
            else
                CurrentObject = await GPSPointApiService.GetByOid(oid: Oid);
        ...
        protected async Task FormSubmit()
        {
            ...
            try
            {
                if (Oid == Guid.Empty)
                {
                    CurrentObject = await GPSPointApiService.Create(CurrentObject);
                }
                else
                {
                    var result = await GPSPointApiService.Update(oid: Oid, CurrentObject);
