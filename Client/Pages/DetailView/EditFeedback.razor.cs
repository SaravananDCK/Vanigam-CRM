...
namespace Vanigam.CRM.Client.Pages.DetailView
{
    ...
    {
        [Inject] private FeedbackApiService FeedbackApiService { get; set; }
        protected override async Task OnInitializedAsync()
        {
            if (Oid == Guid.Empty)
                CurrentObject = new();
            else
                CurrentObject = await FeedbackApiService.GetByOid(oid: Oid);
        ...
        protected async Task FormSubmit()
        {
            ...
            try
            {
                if (Oid == Guid.Empty)
                {
                    CurrentObject = await FeedbackApiService.Create(CurrentObject);
                }
                else
                {
                    var result = await FeedbackApiService.Update(oid: Oid, CurrentObject);
